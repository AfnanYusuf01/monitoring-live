...
//             webpackJsonp([], { parasite: (x, y, z) => getStore(z) }, [
//                 "parasite",
//             ]);
//         } else {
      ...
      neededObjects.forEach((needObj) => {
        if (needObj.foundedModule) {
          if (needObj.id == "ChatCollection") {
            window.Store[needObj.id].ChatCollection.findImpl =
              window.Store[needObj.id].ChatCollection._find;
          }
        }
      });
...
window.WAPI.sendTyping = async function (chatId, done) {
  // await window.Store.WapQuery.sendPresenceAvailable()
...
window.WAPI.getAllGroups = function (done) {
  const groups = window.Store.Chat._models.filter(
    (chat) => chat.__x_id.server === "g.us"
  );
...
window.WAPI.getChatCreate = function (id, done) {
  ...
  if (WAPI.isBeta()) {
    ...
    if (chat) {
      ...
      // Create user
      var idx = new window.Store.UserConstructor(id, {
        intentionallyUsePrivateConstructor: true,
      });

      window.Store.Chat.find(idx)
        .then((chat) => {
          if (done) done(chat);
        })
        .catch((e) => {
...
window.WAPI.getChat = function (id, done) {
  ...
  if (WAPI.isBeta()) {
    ...
    if (chat) {
      ...
      window.Store.Chat.find(idx).then(chat => {
        if (done) done(chat);
      }).catch(e => {
        if (done) done(null);
      }) **/
...
window.WAPI.sendMessageToIDOld = function (id, message, done) {
  let chat = Store.Chat.find(Store.WidFactory.createWid(id));
  ...
  try {
    ...
    if (WAPI.isBeta()) {
      ...
      try {
        ...
        firstChat = Store.Chat._models[0];
...
window.WAPI.findJidFromNumber = (number) => {
  if (WAPI.isBeta()) {
    return Store.QueryExist.queryExist(WAPI.tryFixNumber("+" + number)).then(
      (value) => {
...
window.WAPI.tryFixNumber = (number) => {
  let firstNumbersMe = Store.GetMaybeMeUser.getMaybeMeUser().user.substring(
    0,
    2
  );
...
window.WAPI.getmsg = function (done) {
  var text = "";
  var hasil = "";
  var y = window.WAPI.getBufferedNewMessages();
  for (i = 0; i < y.length; i++) {
    text += y[i].from.user + "newargs" + y[i].content + "newline";
  }
  hasil = text;
  text = "";
  return hasil;
...
window.WAPI.sendVCard = function (chatId, vcard) {
  ...
  var tempMsg = Object.create(
    Store.Msg._models.filter((msg) => msg.__x_isSentByMe)[0]
  );
